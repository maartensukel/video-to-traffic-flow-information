...
def process_result(detection, obj_threshhold, nms_threshhold, relevant_classes_indices, class_mapping):
    """
    Do objectness score filtering and non max suppression
    """
    ...
    for batchi in range(detection.size(0)):
        ...
        # bboxes has the format (x1, y1, x2, y2, obj_prob, cls_prob_1, cls_prob_2, ..., cls_prob_n)
        ...
        # Filter out and remap classes to improve NMS
        relevant_ind = np.isin(bboxes[:, -1].detach(), relevant_classes_indices)
        bboxes = bboxes[relevant_ind]
        for to_class, from_classes in class_mapping.items():
            from_ind = np.isin(bboxes[:, -1].detach(), from_classes)
            bboxes[from_ind, -1] = to_class
...
def transform_result(detections, imgs, input_size):
    """
    transform bounding box position in the resized image(input image to the network) to the corresponding position in
    the original image
    """
